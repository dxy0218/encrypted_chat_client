            ...
            if message.startswith("/emoji"):
                ...
                if len(parts) == 1 or parts[1] == "list":
                    ...
                    if path and os.path.exists(path):
                        ...
                        msg_id = state["counter"]
                        state["counter"] += 1
                        history.append({"id": msg_id, "dir": "out", "type": "image", "name": os.path.basename(path), "time": time.time()})
                    else:
                ...
                msg_id = state["counter"]
                state["counter"] += 1
                history.append({"id": msg_id, "dir": "out", "type": "video", "name": os.path.basename(path), "time": time.time()})
                ...
                msg_id = state["counter"]
                state["counter"] += 1
                history.append({"id": msg_id, "dir": "out", "type": "file", "name": os.path.basename(path), "time": time.time()})
...
def contacts_mode(default_port):
    ...
    while True:
        ...
        if visible:
            for name, addr in visible.items():
        ...
        if choice == "add":
            name = input(f"{COLOR_INPUT}Name: {COLOR_RESET}")
            token_or_ip = input(
            ...
            if data:
                ...
                contacts[name] = {"host": host, "port": port, "id": token_or_ip}
            else:
                contacts[name] = {"host": token_or_ip, "port": port}
            save_contacts(contacts)
            print(f"{COLOR_INFO}Contact {name} added.{COLOR_RESET}")
        elif choice == "remove":
            name = input(f"{COLOR_INPUT}Name to remove: {COLOR_RESET}")
            if name in contacts:
                contacts.pop(name)
                save_contacts(contacts)
                print(f"{COLOR_INFO}Contact {name} removed.{COLOR_RESET}")
            else:
                print(f"{COLOR_WARN}Contact not found.{COLOR_RESET}")
        elif choice == "hide":
            name = input(f"{COLOR_INPUT}Name to hide: {COLOR_RESET}")
            if name in contacts:
                contacts[name]["hidden"] = True
                save_contacts(contacts)
                print(f"{COLOR_INFO}Contact {name} hidden.{COLOR_RESET}")
            else:
                print(f"{COLOR_WARN}Contact not found.{COLOR_RESET}")
        elif choice == "unhide":
            name = input(f"{COLOR_INPUT}Name to unhide: {COLOR_RESET}")
            if name in contacts:
                contacts[name]["hidden"] = False
                save_contacts(contacts)
                print(f"{COLOR_INFO}Contact {name} unhidden.{COLOR_RESET}")
            else:
                print(f"{COLOR_WARN}Contact not found.{COLOR_RESET}")
        elif choice == "exit":
            break
...
def main():
    ...
    user_id = load_or_create_profile(args.port)
